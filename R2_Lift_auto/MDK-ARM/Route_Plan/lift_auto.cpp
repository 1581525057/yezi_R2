...
void LiftAuto::update(void)
{
    ...
    // 读取DT35激光测距数据（通道0，经ADS8688 ADC采样换算）
    const uint32_t laser_mm = (uint32_t)dt35.ch0.distance_mm; // 当前距离（mm），float转uint32_t
    const uint8_t laser_valid = dt35.ch0.valid;               // 数据是否有效
