...
DM_IMU::DM_IMU()
{
    ...
    memset(dmimu_data, 0, sizeof(dmimu_data));
}

const uint16_t DM_IMU::CRC16_table[256] =
    {
...
// CRC16计算函数
uint16_t DM_IMU::Get_CRC16(uint8_t *ptr, uint16_t len)
{
    uint16_t crc = 0xFFFF;
...
// 字节转浮点数（小端模式）
float DM_IMU::bytesToFloat(uint8_t *bytes)
{
    union {
        uint32_t u32;
        float f;
    ...
    converter.u32 = (uint32_t)bytes[0] |
                    (uint32_t)bytes[1] << 8 |
                    (uint32_t)bytes[2] << 16 |
                    (uint32_t)bytes[3] << 24;
    return converter.f;
...
// 解析单个数据包（19字节）的通用函数
// 注意：这个函数不区分数据类型，而是根据数据包中的寄存器ID来解析
ParseResult DM_IMU::ParseSingleIMUPacket(uint8_t *data)
{
    // 校验帧头和帧尾
    if (data[0] != 0x55 || data[1] != 0xAA || data[18] != 0x0A) {
        return ParseResult::HeaderError;
    }

    // CRC校验
    uint16_t calcCRC = Get_CRC16(data, 16);
    uint16_t recvCRC = (data[17] << 8) | data[16];
    if (calcCRC != recvCRC) {
    ...
    switch (data[3]) {
        case 0x01: // 加速度
            imu.accel_ax = bytesToFloat(&data[4]);
            imu.accel_ay = bytesToFloat(&data[8]);
            imu.accel_az = bytesToFloat(&data[12]);
            break;
        case 0x02: // 角速度
            imu.gyro_gx = bytesToFloat(&data[4]);
            imu.gyro_gy = bytesToFloat(&data[8]);
            imu.gyro_gz = bytesToFloat(&data[12]);
            break;
        case 0x03: // 欧拉角
            imu.roll  = bytesToFloat(&data[4]);
            imu.pitch = bytesToFloat(&data[8]);
            imu.yaw   = bytesToFloat(&data[12]);
            break;
        default:
...
// 解析57字节的总数据流
void DM_IMU::ParseIMUStream(uint8_t *streamData)
{
    // 分别解析
    ParseSingleIMUPacket(streamData);
